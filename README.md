...
| `portal-host-mode` | <p>How to expose the portal (self-hosted only; other values are ignored)</p> | `false` | `self-hosted` |
| `selfhosted-listen-address` | <p>Address and port the HTTP server should bind to while in `self-hosted` mode</p> | `false` | `:8080` |
| `selfhosted-public-url` | <p>The public URL (for example, behind an AWS ALB) used to reach the portal when `self-hosted` mode is selected</p> | `false` | `""` |
| `github-token` | <p>The token used to authenticate with GitHub API</p> | `true` | `${{ github.token }}` |
| `runner-endpoint-key` | <p>Optional unique key to namespace endpoints for this runner (e.g., <code>run-12345</code>)</p> | `false` | `""` |
...
## Unique Runner Endpoint

- Use `runner-endpoint-key` to serve each runner’s portal under a unique URL segment. This ensures requests are routed to the correct runner instance.
- Recommended values (URL-safe, no spaces or slashes):
  - `${{ github.run_id }}`
  - `${{ github.run_id }}-${{ github.job }}`
  - `${{ github.run_id }}-${{ github.job }}-${{ matrix.os }}`
    ...
    steps:
      - name: Interactive Inputs
        ...
        with:
          ...
          runner-endpoint-key: ${{ github.run_id }}-${{ github.job }}
          interactive: |
            fields:
...
Example (default key derived)

If you omit `runner-endpoint-key`, the action derives a key like `input-<shortSHA>-<RunID>` from the GitHub context. With `selfhosted-public-url` set to your domain root, links look like `https://alb.example.com/input-<shortSHA>-<RunID>/`.
...
jobs:
  interactive-inputs:
    runs-on: ubuntu-latest
    steps:
      - name: Interactive Inputs (default key)
        uses: boasihq/interactive-inputs@v2
        with:
          selfhosted-listen-address: ":8080"
          selfhosted-public-url: "https://alb.example.com"
          github-token: ${{ secrets.GITHUB_TOKEN }}
          interactive: |
            fields:
              - label: requested-files
...
Resulting endpoints (example)

- UI: `https://alb.example.com/input-<shortSHA>-<RunID>/`
- Submit: `https://alb.example.com/input-<shortSHA>-<RunID>/submit`
- Cancel: `https://alb.example.com/input-<shortSHA>-<RunID>/cancel`
- Upload: `https://alb.example.com/input-<shortSHA>-<RunID>/api/v1/upload`
- Reset: `https://alb.example.com/input-<shortSHA>-<RunID>/api/v1/reset/{inputLabel}`

Resulting endpoints (for key `abc123`)

- UI: `/abc123/`
- Submit: `/abc123/submit`
- Cancel: `/abc123/cancel`
- Upload: `/abc123/api/v1/upload`
- Reset: `/abc123/api/v1/reset/{inputLabel}`

Defaults if omitted

- GitHub Actions: auto-derives `input-<shortSHA>-<RunID>` if SHA is available (falls back to `run-<RunID>`).
- Local/test: defaults to `runner` (or `local` when `IAIP_SKIP_CONFIG_PARSE=1`).

## Migration Notes

- Namespaced endpoints
  - All routes are now served under `/<runner-endpoint-key>/...`.
  - Backward compatibility: the server permanently redirects `/` and `/<key>` to `/<key>/`.
  - If you have automation calling older paths directly, update them to include the key.

- Notifications
  - Slack/Discord links now include the base path so they land users on the correct runner instance.

- Self-hosted setups
  - Ensure any reverse proxy/load balancer allows dynamic path segments (e.g., `^/(.+?)/...`).
  - No change needed for static assets (`/static/...`).

- Choosing a key
  - Prefer `${{ github.run_id }}` or `${{ github.run_id }}-${{ github.job }}` to avoid collisions.
  - Do not include spaces or slashes.
  
- Portal host mode
  - `portal-host-mode` is accepted for compatibility. Only `self-hosted` is supported.
  - Any other value is ignored and a warning is logged; the action proceeds in self-hosted mode.

## Self-hosted Reverse Proxy Examples

When running in self-hosted mode (the only mode), you may want to expose the portal under a URL prefix (for example, `/inputs`). Set `selfhosted-public-url` to that prefix (e.g., `https://alb.example.com/inputs`) and configure your proxy to strip that prefix before forwarding to the app. The app expects `/<runner-endpoint-key>/...` at the upstream.
...
server {
  ...
  # Forward /inputs/* to the app and strip "/inputs" prefix
  location /inputs/ {
    proxy_set_header Host $host;
    proxy_set_header X-Forwarded-Proto $scheme;
    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
    proxy_set_header X-Forwarded-Prefix /inputs;
    proxy_http_version 1.1;
    proxy_buffering off;
    proxy_read_timeout 65s;

    # Trailing slash here strips '/inputs' so upstream sees '/<key>/...'
...
```apache
<VirtualHost *:443>
  ServerName alb.example.com

  ProxyPreserveHost On
  ProxyRequests Off
  # Map /inputs/* to upstream root, stripping '/inputs'
  ProxyPass        /inputs/ http://127.0.0.1:8080/
  ProxyPassReverse /inputs/ http://127.0.0.1:8080/
</VirtualHost>
```

Traefik (Static + Dynamic via labels)

```yaml
labels:
  - "traefik.enable=true"
  - "traefik.http.routers.inputs.rule=Host(`alb.example.com`) && PathPrefix(`/inputs`)"
  - "traefik.http.routers.inputs.entrypoints=websecure"
  - "traefik.http.middlewares.inputs-strip.stripprefix.prefixes=/inputs"
  - "traefik.http.routers.inputs.middlewares=inputs-strip"
  - "traefik.http.services.inputs.loadbalancer.server.port=8080"
```

Notes
- No changes are needed for `/static/...`; the proxy configuration above forwards those paths as well.
- Ensure redirects are allowed (we return 308 from `/` to `/<key>/`).
...
### Operating without ngrok (self-hosted / AWS ALB)

If you prefer to expose the portal via your own networking stack (for example, an AWS Application Load Balancer in front of a self-hosted runner), set `portal-host-mode: self-hosted`. In this mode the action launches an HTTP server that binds to `selfhosted-listen-address` (defaults to `:8080`). You are responsible for routing traffic to that port; a common pattern is to register the runner instance in an ALB target group or Kubernetes service so a controller can provision an external address. The URL that users should visit (and the URL sent in Slack/Discord notifications) is taken from `selfhosted-public-url`.

```yaml
      - name: Example Interactive Inputs Step (self-hosted)
        uses: boasihq/interactive-inputs@v2
        with:
          portal-host-mode: self-hosted
          selfhosted-listen-address: "0.0.0.0:9090"   # port your ALB forwards traffic to
          selfhosted-public-url: https://alb.example.com/inputs
          github-token: ${{ github.token }}
          interactive: |
            fields:
              - label: change-window
                properties:
                  display: Select a change window
                  type: select
                  choices: ["APAC", "EMEA", "AMER"]
```

This approach allows you to keep traffic inside your network perimeter, reuse existing TLS certificates on the load balancer, and removes the dependency on ngrok entirely.
